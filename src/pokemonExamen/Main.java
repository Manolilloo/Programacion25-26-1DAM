...
import pokemonExamen.exceptions.ValorNoValidoException;
import pokemonExamen.interfaces.Atacable;
import pokemonExamen.models.*;
import pokemonExamen.utils.UserDataCollector;
...
public class Main {
    public static void main(String[] args) {
        ...
        for (Pokemon pokemon : pokemons) {
            System.out.println(pokemon.getNombre() + " - " + pokemon.getClass().getSimpleName() + " - " + pokemon.getPtsSalud() + " puntos de salud");
        }

        while (!hayGanador(pokemons)){
            pokemonExamen.enums.WeatherCondition weatherCondition = generateRandomWeatherCondition();
            System.out.println();
            System.out.println("##################################################");
            System.out.println("##################################################");
            ...
            for (int i = 0; i < pokemons.length; i++) {
                ...
                if (pokemon.estaVivo()) {
                    ...
                    try {
                        ...
                        System.out.println(pokemon.getNombre() + " ha atacado a " + ((Pokemon) atacable).getNombre() + " - " + ((Pokemon) atacable).getPtsSalud() + " puntos de salud");
    ...
    private static Atacable seleccionaPokemonAtacable(Pokemon[] pokemons, int i) {
        ...
        for (int j = 0; j < pokemons.length; j++) {
            if (pokemons[j].estaVivo() && j != i && pokemons[j] instanceof Atacable){
                System.out.println((numPokemonVivos + 1) + ": " + pokemons[j].getNombre() + " - " + pokemons[j].getClass().getSimpleName() + " - " + pokemons[j].getPtsSalud() + " puntos de salud");
                numPokemonVivos++;
    ...
    private static boolean hayGanador(Pokemon[] pokemons){
        int contador = 0;
        for (int i = 0; i < pokemons.length; i++) {
            if (!(pokemons[i] instanceof PokemonDivino) && pokemons[i].getPtsSalud() > 0){
                contador++;
            }
        }
        return contador == 1;
